...
function ResponsiveGroup({ children, position = [0, 0.38, 0] }: { children: React.ReactNode, position?: [number, number, number] }) {
    ...
    setTimeout(() => {
      ...
      if (ctxC && ctxB) {
        ...
        for (let i = 0; i < 2000; i++) {
          ...
          ctxC.fillStyle = '#f2f2f4';
          ctxC.fill();
          
          ctxB.beginPath();
          ctxB.arc(x, y, r, 0, Math.PI * 2);
          ctxB.fillStyle = '#858585';
          ctxB.fill();
        }
      }
...
function RobotPrototype({ neckParams = { baseR: 0.25, baseH: -0.01, midR: 0.23, midH: 0.02, lipBottomR: 0.27, lipBottomH: 0.025, lipTopR: 0.28, lipTopH: 0.05, innerR: 0.24, innerDropH: 0.03 }, bodyParams = { bodyBevelR: 0.21, bodyBevelY: 0.38, bodyBevelT: 0.015 } }: { neckParams?: Record<string, number>, bodyParams?: Record<string, number> }) {
  ...
  useFrame((state, delta) => {
    ...
    const tx = mouseRef.current.x !== 0 || mouseRef.current.y !== 0 ? mouseRef.current.x : state.pointer.x;
    const ty = mouseRef.current.x !== 0 || mouseRef.current.y !== 0 ? mouseRef.current.y : state.pointer.y;
          ...
          <div className="flex flex-wrap justify-center lg:justify-start items-center gap-2 sm:gap-3 z-20">
            {leftItems.map((item, idx) => (
              <a
                ...
                className="relative px-7 py-2.5 rounded-full bg-white text-black hover:bg-zinc-200 text-sm font-bold transition-all overflow-hidden shadow-[0_4px_14px_rgba(255,255,255,0.15)] cursor-pointer"
              >
                {item.label}
                {hoveredIndex === idx && (
                  <motion.div
          ...
          <div className="flex flex-wrap justify-center lg:justify-end items-center gap-2 sm:gap-3 w-full lg:w-auto mt-4 lg:mt-0 z-20">
            {contactText ? (
              <a
                ...
                className="px-5 sm:px-7 py-2.5 rounded-full bg-white text-black hover:bg-zinc-200 text-xs sm:text-sm font-bold transition-all shadow-[0_4px_14px_rgba(255,255,255,0.15)] cursor-pointer"
              >
                {contactText}
              </a>
            ) : null}
...
export function RobotHero({
  ...
  return (
    <section 
      ref={containerRef}
      className="relative w-full min-h-screen lg:h-screen lg:max-h-screen overflow-hidden flex flex-col"
    ...
    >
      ...
      <div className="absolute inset-0 z-10">
        ...
        >
          ...
          <ResponsiveGroup position={[0, 0.38, 0]}>
      ...
      <div className="absolute inset-0 z-20 pointer-events-none flex flex-col h-full min-h-screen lg:min-h-0">
        <AntennaNavbar 
        ...
        <div className="relative w-full max-w-[1380px] mx-auto px-6 sm:px-8 flex-1 flex flex-col justify-end pb-6 lg:pb-8 pt-0">
          <div className="w-full pointer-events-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 max-w-[1380px] mx-auto">
            {/* Card 1 */}
            <div className="relative bg-white/[0.08] backdrop-blur-[24px] border border-white/[0.18] rounded-[28px] px-7 py-5 lg:py-6 h-[225px] lg:h-[235px] shadow-[0_20px_50px_rgba(0,0,0,0.12)] transition-all duration-350 ease hover:-translate-y-[10px] hover:backdrop-blur-[30px] hover:border-white/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.18),0_0_30px_rgba(34,245,181,0.25)] flex flex-col justify-start text-left group">
              <div className="w-2.5 h-2.5 rounded-full bg-[#22F5B5] shadow-[0_0_10px_#22F5B5] mb-3.5 shrink-0" />
              <h3 className="font-bold text-white text-[19px] lg:text-[20px] leading-[1.3] mb-2">
                Master every interview type in one platform
              </h3>
              <p className="text-[13.5px] lg:text-[14px] text-white/75 leading-[1.6]">
                Resume, technical, coding, behavioral, and role-specific questions—all tailored to your background.
              </p>
            </div>
            ...
            <div className="relative bg-white/[0.08] backdrop-blur-[24px] border border-white/[0.18] rounded-[28px] px-7 py-5 lg:py-6 h-[225px] lg:h-[235px] shadow-[0_20px_50px_rgba(0,0,0,0.12)] transition-all duration-350 ease hover:-translate-y-[10px] hover:backdrop-blur-[30px] hover:border-white/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.18),0_0_30px_rgba(34,245,181,0.25)] flex flex-col justify-start text-left group">
              ...
              <p className="text-[13.5px] lg:text-[14px] text-white/75 leading-[1.6]">
                Practice unlimited interviews with Gemini-powered evaluation that learns your strengths and identifies improvement areas.
              </p>
            </div>
            ...
            <div className="relative bg-white/[0.08] backdrop-blur-[24px] border border-white/[0.18] rounded-[28px] px-7 py-5 lg:py-6 h-[225px] lg:h-[235px] shadow-[0_20px_50px_rgba(0,0,0,0.12)] transition-all duration-350 ease hover:-translate-y-[10px] hover:backdrop-blur-[30px] hover:border-white/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.18),0_0_30px_rgba(34,245,181,0.25)] flex flex-col justify-start text-left group">
              ...
              <p className="text-[13.5px] lg:text-[14px] text-white/75 leading-[1.6]">
                Our AI analyzes your resume to generate personalized interview questions based on your skills, projects, experience, and target role.
              </p>
            </div>
            ...
            <div className="relative bg-white/[0.08] backdrop-blur-[24px] border border-white/[0.18] rounded-[28px] px-7 py-5 lg:py-6 h-[225px] lg:h-[235px] shadow-[0_20px_50px_rgba(0,0,0,0.12)] transition-all duration-350 ease hover:-translate-y-[10px] hover:backdrop-blur-[30px] hover:border-white/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.18),0_0_30px_rgba(34,245,181,0.25)] flex flex-col justify-start text-left group">
              ...
              <p className="text-[13.5px] lg:text-[14px] text-white/75 leading-[1.6]">
                Receive real-time feedback, ideal answers, performance analytics, and actionable insights that help you succeed in real interviews.
              </p>
            </div>
          </div>
        </div>
